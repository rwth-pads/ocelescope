...
from plugin.schema import MethodDefinition, OcelInput, OutputDefinition
...
class ExamplePlugin(PluginBase):
    @classmethod
    def plugin_methods(cls):
        return {
            "ocpn": MethodDefinition(
                input_types=[
                    OcelInput(
                        type="ocel",
                        name="source_ocel",
                        description="The Ocel that should be used",
                    )
                ],
                output_types=OutputDefinition(
                    type="resource", name="output", resource_types="ocpn"
                ),
                run=cls.ocpn,
            ),
            "ocdfg": MethodDefinition(
                input_types=[
                    OcelInput(
                        type="ocel",
                        name="source_ocel",
                ...
                output_types=OutputDefinition(
                    type="resource", name="output", resource_types="ocdfg"
                ),
                run=cls.ocdfg,
            ),
        }

    def ocpn(self, inputs):
        ocel: OCELWrapper = inputs["source_ocel"]

        petri_net = pm4py.discover_oc_petri_net(ocel.ocel)
        petri_net = convert_flat_pm4py_to_ocpn(petri_net["petri_nets"])
        return {"ocpn": petri_net}

    def ocdfg(self, inputs):
        ocel: OCELWrapper = inputs["source_ocel"]
