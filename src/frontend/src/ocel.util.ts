...
export function getObjectTypeActivities(ocel: OCEL, objectType: string | undefined) {
  return objectType ? ocel.activities.filter(act => objectType in ocel.e2oCounts[act]) : []
}
export function getActivityObjectTypes(ocel: OCEL, activity: string | undefined) {
  return activity ? Object.keys(ocel.e2oCounts[activity] ?? {}) : []
}
export function getQualifiers(ocel: OCEL, { activity, objectType }: { activity?: string, objectType?: string } = {}) {
  ...
  if (objectType) {
    return _.uniq(actQs.map(actQs => Object.keys(actQs[objectType] ?? {})).flat(1))
  }
...
export async function processOcelUpload(file: File) {
  ...
  if (ext == "sqlite") {
    ...
    const ocelFileNames = fileNames.filter(f => f.split(".").at(-1) == "sqlite")
    if (!ocelFileNames.length) {
      return { error: "The selected zip file does not contain a .sqlite object-centric event log." }
    }
    if (ocelFileNames.length > 1) {
      return { error: "The selected zip file does not contain a unique .sqlite object-centric event log." }
    }
    const ocelFileName = ocelFileNames[0]
    const jsonFileName = `${ocelFileNames}.meta.json`
    if (!zip.files[jsonFileName]) {
      return { error: "The selected zip file does not contain JSON preferences." }
    }
    let appState
    try {
      appState = JSON.parse(await zip.files[jsonFileName].async("text")) as unknown as AppStateExport
    } catch (err) {
