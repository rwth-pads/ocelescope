...
import { ApiF, ApiRequestOptions, ApiResponseType, ApiWrapper } from "@/src/api.types";
import { AppState_Output, E2OEmissionRule_Output, EventAttributeDefinition, EventEmissionRule_Output, ObjectAllocationConfig, ObjectAttributeDefinition } from "@/src/api/generated";
import { OCEL, OCELAttribute, isEventAttribute, isObjectAttribute } from "@/src/ocel.types";
...
export type AttributeDefinition = EventAttributeDefinition | ObjectAttributeDefinition
...
export type ReplaceTypeRecursively<T, S, R> = T extends S ? R : (T extends object ? {
  [P in keyof T]: ReplaceTypeRecursively<T[P], S, R>
} : T)

export type AppStateExport = ReplaceTypeRecursively<AppState, chroma.Color, string>
 ...
 * @returns 
 */
export function buildOcelAttributeUnitDefinitions(attributes: OCELAttribute[]): AttributeDefinition[] {
...
export function findOcelAttributeUnitDefinitionIndex(
  ...
  return attributeUnits.findIndex(au => {
    ...
    if (au.target == "object" && isObjectAttribute(attr) && au.objectType == attr.objectType && au.dynamic == attr.dynamic) return true
    if (au.target == "event" && isEventAttribute(attr) && au.activity == attr.activity) return true
    return false
  })
}
...
export function findOcelAttribute(
  ocel: OCEL,
  ...
  return ocel.attributes.find(attr => {
    ...
    if (attr.target == "event" && def.target == "event" && attr.activity == def.activity) return true
    if (attr.target == "object" && def.target == "object" && attr.objectType == def.objectType) {
...
export function useAttributeUnit(
  ...
  const index = findOcelAttributeUnitDefinitionIndex(attr, attributeUnits)
  const attributeUnitDefinition = index !== -1 ? attributeUnits[index] : undefined
...
export function useAttributeUnitState(
  ...
  const setUnit = (unit: Unit | null | undefined) => setAttributeUnits(aus => {
...
export function exportAppState(appState: AppState): AppStateExport {
  const attributeUnits = appState.attributeUnits?.filter(au => !!au.unit) ?? []
  return {
    objectTypeColors: appState.objectTypeColors ? Object.fromEntries(Object.entries(appState.objectTypeColors).map(
    ...
    emissionAttributes: appState.emissionAttributes.length ? appState.emissionAttributes : [],
...
export async function applyImportedAppState(
  ...
  if (appState.objectTypeColors) {
    updates.objectTypeColors = Object.fromEntries(Object.entries(appState.objectTypeColors).map(
      ([objectType, hex]) => [objectType, chroma(hex)]
    ))
  }
  ...
  const ocelAttributeUnitDefinitions = ocel.attributes.map(attr => findOcelAttributeUnitDefinition(attr, aus))
  const attributesWithoutUnitDefinition = ocel.attributes.filter((attr, i) => ocelAttributeUnitDefinitions[i] === undefined)
  const existingImportedUnitDefinitions = ocelAttributeUnitDefinitions.filter(au => au) as AttributeDefinition[]
  const emptyAttributeUnitDefinitions = buildOcelAttributeUnitDefinitions(attributesWithoutUnitDefinition)
  ...
  if (appState.emissionRules) {
    updates.emissionRules = appState.emissionRules.filter(ec => !isEmissionRuleEmpty(ec))
  }
...
export async function apiRequestSequence<F extends ApiF>(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    console.log(`apiRequestSequence: execute request (${restRequests.length} remaining after this)`)
    ...
    apiWrapper(request, {
      ...
      onCompletion: async response => {
        ...
        const restResponses = await apiRequestSequence(apiWrapper, restRequests, restOptions, commonOptions)
