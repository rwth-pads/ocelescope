...
export function updateObjectTypeColors(
  ...
  setObjectTypeColors((prevColors) => {
    ...
    if (
      prevColors &&
      objectTypeSubset.length == objectTypesWithColor.length &&
      objectTypeSubset.every((ot) => objectTypesWithColor.includes(ot))
    )
      return prevColors;
    // Generate new colors
    const palette = generateColorPalette(objectTypeSubset.length);
    return Object.fromEntries(
      objectTypeSubset.map((ot, i) => [ot, palette[i]]),
    );
  });
}
...
export function generateColorPalette(numColors: number): chroma.Color[] {
  ...
  const colors = hues.map((h) =>
    chroma.hsl(
      h + _.random(-3, 3), // hue
      _.random(0.6, 0.9), // saturation
      _.random(0.2, 0.5), // value (brightness)
    ),
  );
  return colors;
}
