...
export type TaskResultType<F extends ApiF> = Exclude<
  ApiResponseType<F>,
  TaskStatusResponse
>;
...
export type ApiWrapper<F extends ApiF> = (
  exec: ApiExec<F>,
  options: ApiRequestOptions<F>,
) => Promise<ApiResponseType<F> | false>;
...
export function findRunningTask(
  ...
  filters: Partial<BackendTask<ApiF>>,
) {
  return tasks.findLast((task) => {
    if (!(task.taskState != "SUCCESS" && task.taskState != "FAILURE"))
      return false;
...
export type TypedTaskStatusResponse<TRes extends object> = Omit<
  TaskStatusResponse,
  "task"
> & {
...
export function scheduleTaskPoll<F extends ApiF>(
  ...
  const { setTask, onCompletion } = options;
  ...
  setTimeout(async () => {
    ...
    const data = await apiWrapper(
      ...
      },
      {
        // [task-status]
        skipMutex: true,
        setTask: setTask as unknown as ApiRequestOptions<
          typeof Api.taskStatusTaskStatusGet
        >["setTask"],
