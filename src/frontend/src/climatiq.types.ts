...
export type DataQualityFlag =
  ...
  | "partial_factor"
  | "self_reported"
  | "suspicious_homogeneity"
  | "erroneous_calculation";
export const defaultAllowedDataQualityFlags: DataQualityFlag[] = [
  "notable_methodological_variance",
  "partial_factor",
];
...
export async function climatiqDataVersionRequest(
  ...
  const responseData = (await runClimatiqRequest(
    "data-versions",
    {},
    apiKey,
  )) as ClimatiqDataVersionsResponse | false;
...
export async function climatiqSearchRequest(
  ...
  const apiKey = validateClimatiqApiKey(config);
  const dataVersion =
    config.dataVersion ?? process.env.NEXT_PUBLIC_CLIMATIQ_DATA_VERSION;
  ...
  const unknownFilters = Object.keys(filters).filter(
    (k) =>
      !ALLOWED_SINGLE_FILTERS.includes(k) && !ALLOWED_LIST_FILTERS.includes(k),
  );
  if (unknownFilters.length) {
    console.error(
      `The following parameters are not defined in the climatiq "search" API: ` +
        unknownFilters.join(", "),
    );
    return false;
  }
        ...
        .map(([key, value]) => {
          ...
          if (Array.isArray(value)) {
            ...
            if (!ALLOWED_LIST_FILTERS.includes(key)) {
              console.error(
                `The parameter "${key}" only accepts a single value in the climatiq "search" API, received list-like ${JSON.stringify(value)}`,
              );
              return [undefined, undefined];
            }
          ...
          console.error(
            `The parameter "${key}" has a wrong type to be passed to the climatiq "search" API: "${typeof value}"`,
          );
          return [undefined, undefined];
        })
  ...
  const responseData = (await runClimatiqRequest("search", formData, apiKey)) as
  ...
  responseData.results = responseData.results
    ...
    .map((res) => {
      ...
      return {
        ...
        constituentGases: removeEmptyProperties(constituentGases, {
          removeZero: true,
        }),
      };
    }) as ClimatiqEmissionFactor[];
  responseData.possibleFilters = objectKeysToCamelCase(
    responseData.possibleFilters,
  ) as ClimatiqPossibleFiltersList;

  if (!("possibleFilters" in responseData)) throw Error();
  responseData.possibleFilters.region = responseData.possibleFilters.region.map(
    ({ id, name }) => initRegion(id, name),
  );
...
async function runClimatiqRequest(
  ...
  return await climatiqApiMutex.runExclusive(async () => {
    ...
    if (!response.ok) {
      ...
      console.error(
        `Climatiq API call to "${route}" failed (Error ${response.status} ${response.statusText}: ${responseText}).`,
      );
    ...
    if ("error" in responseData) {
      console.error(
        `Climatiq API call to "${route}" failed (${[responseData.error, responseData.errorCode].filter((x) => x).join(" / ")}: ${responseData.message}`,
      );
      return false;
    }
...
export function createAllUnits(
  unitTypes: ClimatiqUnitType[] | undefined,
): (Unit | null)[] {
  if (!unitTypes) return [];
  ...
  return unitTypes
      ...
      if (ut.units) {
        return ut.units.map((u) =>
          u
            ...
            : null,
        ); // TODO null is only returned for ut "Number". Might still change to Unit obj with unit = undefined.
      }
      if (ut.composedUnits && ut.composedUnitTypes) {
        const uts = ut.composedUnitTypes;
        const utUnits = ut.composedUnits;
        if (
          ut.composedUnitTypes.length != 2 ||
          Object.values(ut.composedUnits).length != 2
        ) {
          console.error(
            `Only supporting product unit types of up to two factors (encountered ${uts.join(", ")})`,
          );
          return undefined;
        }
        return (utUnits[uts[0]] ?? [null])
          .map((u1) => {
            return (utUnits[uts[1]] ?? [null]).map((u2) => {
              if (!u1 && !u2) {
                console.error(
                  `Composed unit types should have at least one physical (continuous) unit type (encountered ${uts.join(", ")})`,
                );
                return undefined;
              }
    ...
    .flat(1)
    .filter((u) => u !== undefined) as (Unit | null)[];
}
...
export async function loadClimatiqUnitTypes(): Promise<ClimatiqUnitType[]> {
  ...
  const unitTypes: (ClimatiqUnitType | undefined)[] = unitTypesData.map(
    ({ unitType, units }) => {
      ...
      if (multParts.length == 1) {
        ...
        console.error(
          `Only supporting product unit types of up to two factors (encountered ${multParts.join(", ")})`,
        );
        return undefined;
      }
      return ut;
...
export function sortClimatiqUnitTypes(climatiqName: string | null | undefined) {
  ...
  const partImportance = importantUnitTypes.findIndex((u) =>
    climatiqName.toLowerCase().includes(u),
  );
...
export const climatiqMacroRegions = [
  ...
  {
    id: "ROW_WF",
    name: "Africa except Egypt and South Africa",
    Icon: FaEarthAfrica,
  },
  { id: "ASIA_AFRICA", name: "Asia and Africa", Icon: FaEarthAsia },
  {
    id: "ROW_WA",
    name: "Central Asia and Pacific Asia, Oceania, Antarctica",
    Icon: FaEarthAsia,
  },
