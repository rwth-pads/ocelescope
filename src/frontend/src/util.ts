...
import { DependencyList, Dispatch, RefObject, SetStateAction, useEffect, useMemo, useRef, useState } from "react";

export function useComputedStyle<T extends Element>(key: string, ref: RefObject<T>) {
  const value = useMemo(() => {
    if (!ref?.current) {
      return null
    }
 ...
 */
export function refreshState<S>(setState: Dispatch<SetStateAction<Refreshable<S>>>) {
  setState(prev => {
    if (!prev) {
      return prev
    }
 ...
 * @returns 
 */
export function useRefreshableState<S extends Object>(initialValue: S | undefined = undefined): [
  Refreshable<S>,
  (value: SetStateAction<S>) => Promise<void>,
  () => Promise<void>
] {

  const [state, setState] = useState<Refreshable<S>>(!!initialValue ? {
    ...initialValue,
    refreshCounter: 0
  } : initialValue)
  ...
  const setStateWrapper = async (setStateArg: S | ((prevState: S) => S)) => {
    ...
    await asyncWaitForNewState(() => countRef.current, () => {
      setState(prev => {
        const newState: Refreshable<S> = (typeof setStateArg === "function" ? (setStateArg as ((prevState: S | undefined) => S))(prev) : setStateArg)
...
export function objectKeysToSnakeCase(obj: any): any {
  return Object.fromEntries(Object.entries(obj).map(([k, v]) => [_.snakeCase(k), v]))
}

export function removeEmptyProperties(obj: any, options: {
  removeZero?: boolean
} = {}): any {
  return Object.fromEntries(Object.entries(obj).filter(([k, v]) => options.removeZero ? !!v : (v !== undefined && v !== null)))
}

export function objectToUrl(obj: { [k: string]: string | number | boolean | null | undefined }): string {
  const obj1 = removeEmptyProperties(obj) as { [k: string]: string | number | boolean }
  return Object.entries(obj1).map(([key, value]) => encodeURIComponent(key) + "=" + encodeURIComponent(value)).join("&")
}
...
export async function readReadme(locale: string, readFile: (file: string) => Promise<string>) {
  const file = path.join(process.cwd(), "public", `game-info-${(locale).toLowerCase()}.md`)
