...
type CapitalizeStr<S extends string> = S extends `${infer First}${infer Rest}`
...
export type DispatchSetStateActionStateSetters<T> = {
  [K in keyof T as `set${CapitalizeStr<string & K>}`]: Dispatch<
    SetStateAction<T[K]>
  >;
};
...
export function updaterSetter<T, Store>(k: string, set: StoreSetter<Store>) {
  return (updater: (prev: T) => T) =>
    set((state) => Object.fromEntries([[k, updater(_.get(state, k) as T)]]));
}
export function valueOrUpdaterSetter<T, Store>(
  k: string,
  set: StoreSetter<Store>,
) {
  return (valueOrUpdater: SetStateAction<T>) => {
    if (typeof valueOrUpdater === "function") {
      return set((state) =>
        Object.fromEntries([
          [k, (valueOrUpdater as (prev: T) => T)(_.get(state, k) as T)],
        ]),
      );
    }
...
function setterName<State extends object, Action extends object>(
  k: keyof State & string,
) {
  return ("set" + k.charAt(0).toUpperCase() + k.substring(1)) as keyof Action;
}

export function stateAndSetter<T, State extends object, Action extends object>(
  set: StoreSetter<State & Action>,
  k: keyof State & string,
  initialState?: T,
) {
...
export function withSetters<State extends object, Action extends object>(
  set: StoreSetter<State & Action>,
  storeStates: State,
): State & Action {
  const actions = Object.fromEntries(
    Object.entries(storeStates).map(([k, initialState]) => {
      return [
        setterName(k as keyof State & string),
        valueOrUpdaterSetter(k, set),
      ];
    }),
  ) as Action;
  return {
...
type WithStateSelectors<S> = S extends { getState: () => infer T }
  ? S & {
      useState: {
        [K in keyof T]: () => {
          [P in K]: T[K];
        } & DispatchSetStateActionStateSetters<{ [P in K]: T[K] }>;
      };
    }
  : never;
 ...
 */
export const createStateSelectors = <
    ...
    (store.useState as any)[k] = () => {
      return Object.fromEntries([
        ...
        [
          setterName(k as keyof State & string),
          store(
            (s) => s[setterName(k as keyof State & string) as keyof typeof s],
          ),
        ],
      ]);
    };
