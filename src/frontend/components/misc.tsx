...
import { OCELAttribute, isDynamicObjectAttribute, isEventAttribute, isStaticObjectAttribute } from "@/src/ocel.types";
...
import { FaCalendarCheck, FaChartLine, FaCircle, FaCircleDot, FaCirclePlay } from "react-icons/fa6";
...
export const CustomIcon = ({ d, stroke = "currentColor", fill = "currentColor", strokeWidth = "0", height = "1em", width = "1em", ...props }: SVGProps<SVGSVGElement> & {
  d: string
}) => (
  <svg
    viewBox="0 0 512 512" stroke={stroke} fill={fill} strokeWidth={strokeWidth} width={width} height={height} {...props}
  >
    <path id="path" d={d} />
  </svg>
)
...
export const StaticObjectAttributeIcon = ({ ...props }: SVGProps<SVGSVGElement>) => (<CustomIcon d="m 447.92356,239.91124 c 17.67767,0 32.03193,-14.35427 32.03193,-32.03194 0,-17.67767 -14.35426,-32.03193 -32.03193,-32.03193 l -319.55985,0.0707 c -17.67767,0 -32.03193,14.35426 -32.03193,32.03193 0,17.67767 14.35426,32.03193 32.03193,32.03193 l 319.55985,0.0707 z M 64,64 C 64,46.3 49.7,32 32,32 14.3,32 0,46.3 0,64 v 336 c 0,44.2 35.8,80 80,80 h 400 c 17.7,0 32,-14.3 32,-32 0,-17.7 -14.3,-32 -32,-32 H 80 c -8.8,0 -16,-7.2 -16,-16 z" {...props} />)


export function getAttributeNameAndIcon(attr: OCELAttribute) {
  if (isEventAttribute(attr)) return { attrTypeDescription: "Event attribute", AttrTypeIcon: EventIcon }
  if (isDynamicObjectAttribute(attr)) return { attrTypeDescription: "Dynamic object attribute", AttrTypeIcon: DynamicObjectAttributeIcon }
  if (isStaticObjectAttribute(attr)) return { attrTypeDescription: "Static object attribute", AttrTypeIcon: StaticObjectAttributeIcon }
  return {}
}
...
export const objectTypeColorProps = (objectTypeColors: ObjectTypeColors | null, objectType?: string) => ({
  color: (objectTypeColors && objectType && objectType in (objectTypeColors ?? {})) ? objectTypeColors[objectType].css() : undefined,
  className: !objectType || !(objectType in (objectTypeColors ?? {})) ? "text-secondary" : undefined
})
...
export const ObjectTypeIcon: React.FC<{
  ...
  const { icon, tooltip } = (() => {
    ...
    if (objectTypeClass !== undefined ? objectTypeClass == "handling_unit" : (handlingUnits?.includes(objectType) ?? true)) {
      return {
        icon: <HandlingUnitIcon {...colorProps} />,
        tooltip: `${objectType} (handling unit)`
      }
    }
    if (objectTypeClass !== undefined ? objectTypeClass == "resource" : (resources?.includes(objectType) ?? false)) {
      return {
...
export const ObjectTypeWithIcon: React.FC<{
  ...
  return <div className="d-inline-flex align-items-center gap-2">
    <ObjectTypeIcon objectType={objectType} objectTypeClass={objectTypeClass} />
    {objectType}
  </div>
}
...
export const CircleButton = styled(({ children, size, ...props }: Omit<ButtonProps, "size"> & { size: number }) => (
  <Button {...props}>
    {children}
  </Button>
))`
...
export const SkeletonIcon = () => (<Skeleton circle={true} width={16} height={16} />)
