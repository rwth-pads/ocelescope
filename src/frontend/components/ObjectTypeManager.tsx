import ChunkedColumns, {
  ChunkedColumnsProps,
} from "@/components/common/ChunkedColumns";
...
const ObjectTypeManager = styled(
  ({ colProps, className, ...props }: ChunkedColumnsProps) => {
    const ocel = useOceanStore.use.ocel();
    const { objectTypeClasses, setObjectTypeClasses } =
      useOceanStore.useState.objectTypeClasses();
    const { handlingUnits, resources } = useOceanStore(
      useHandlingUnitsAndResources,
    );

    const [editingHandlingUnits, setEditingHandlingUnits] = useState<string[]>(
      handlingUnits ?? ocel?.objectTypes ?? [],
    );
    ...
    const saveHandlingUnits = () => {
      setObjectTypeClasses(
        Object.fromEntries(
          ocel.objectTypes.map((ot) => [
            ot,
            editingHandlingUnits.includes(ot) ? "handling_unit" : "resource",
          ]),
        ),
      );
    };
      ...
      <>
        <Form>
          ...
          >
            {_.orderBy(
              ...
              return (
                <div
                  key={i}
                  className="d-flex align-items-center gap-2 ot-manager-item"
                >
                  ...
                  <ButtonGroup>
                    <ToggleButton
                      ...
                      onChange={(e) =>
                        setHandlingUnit(ot, e.target.value == "handling_unit")
                      }
                    >
                      Resource
                    </ToggleButton>
              ...
              /> */}
                  <ObjectTypeWithIcon
                    objectType={ot}
                    objectTypeClass={
                      isHandlingUnit ? "handling_unit" : "resource"
                    }
                  />
                </div>
              );
          ...
          <ButtonToolbar className="mt-2">
            ...
            <Button
              variant="secondary"
              onClick={() => setEditingHandlingUnits(handlingUnits)}
            >
              Reset
            </Button>
          </ButtonToolbar>
...
const LabeledToggleSwitch = styled(
  ({
    offLabel,
    onLabel,
    checked,
    ...
    return (
      <div
        className={combineClassNames(
          "d-flex gap-2 align-items-center me-2",
          checked ? "checked" : undefined,
          className,
        )}
      >
        <span className="off-label" onClick={() => setChecked(false)}>
          {offLabel}
        </span>
        <Form.Check
          checked={checked}
          ...
          {...props}
        />
        <span className="on-label" onClick={() => setChecked(true)}>
          {onLabel}
        </span>
      </div>
    );
