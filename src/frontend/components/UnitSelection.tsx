...
export const UnitEditor: React.FC<UnitEditorProps> = ({
  ...
  return (
    <div className="d-flex align-items-center">
      ...
      {!isEditing && <>
        <UnitLabel unit={value ?? null} />
        <Button variant="link" size="sm" disabled={disabled} onClick={() => setIsEditing(true)}>
          <FaPencil />
        </Button>
      </>}
    </div>
...
const createOption = (u: Unit | null): UnitOption => ({ value: u, label: u?.symbol ?? "---", unit: u })
...
export const UnitSelection: React.FC<UnitSelectionProps> = ({
  ...
  const [selectedOption, setSelectedOption] = useState<UnitOption>(defaultOption)
...
export const UnitTypeIcon: React.FC<{
  climatiqName?: string
  name?: string
} & IconBaseProps> = ({ name, climatiqName, ...props }) => {
  const def = unitTypeData.find((ut) => {
    if (name && ut.name?.toLowerCase() == name.toLowerCase()) return true
    if (climatiqName && ut.climatiqName?.toLowerCase() == climatiqName.toLowerCase()) return true
    return false
  })
  ...
  if (def.icons) {
    return <div className="d-flex align-items-center">
      {def.icons.map((Icon, i) => <Icon key={i} {...props} />)}
    </div>
  }
