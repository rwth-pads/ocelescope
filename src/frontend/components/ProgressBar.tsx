...
> = ({ task, enableEstimation = false, locales, ...props }) => {
  ...
  const percentageNumberFormat = useMemo(
    () =>
  ...
  useEffect(() => {
    if (percentage < 1 && prevTask && task.id != prevTask.id) {
      console.warn(
        `TaskProgressBar cannot change to a different task before completion!`,
      );
      return;
    }
    ...
    let est = Math.min(p + 0.75 * diff, 1);
    let t = ((task.unchangedPercentageCounter ?? 0) + 1) * taskPollDuration;

    // Limit the estimate
    est = Math.min(est, p + 0.25); // est <= p + 25%
    est = Math.min(est, 0.9); // est <= 90%
  ...
  const label = useMemo(
    () =>
      percentage >= 0.05
        ? percentageNumberFormat.format(Math.floor(percentage * 100) / 100)
        : "",
    [percentage],
  );
  const now = useMemo(
    () => (enableEstimation ? estimate : undefined) ?? percentage,
    [enableEstimation, estimate, percentage],
  );
...
export type ProgressBarStyleProps = Omit<
  BsProgressBarProps,
  "color" | "now" | "min" | "max"
> & {
  log?: boolean;
