...
import { ClimatiqEmissionFactorDetails, EventAttributeDefinition, ObjectAttributeDefinition } from "@/src/api/generated";
import { ClimatiqEmissionFactor } from "@/src/climatiq.types";
import { EditingEmissionFactor, EditingEmissionRule } from "@/src/emissions.types";
...
import { GRAM, KILOGRAM, TONNE, unitTypeData, UnitTypeDim } from "@/src/units.types";
...
import { FaArrowUpRightFromSquare, FaBan, FaFilter, FaUserPen } from "react-icons/fa6";
import { ClimatiqLogoIcon } from "../climatiq/ClimatiqConfigModal";
import ClimatiqFactorInput, { ClimatiqUnitTypeFilterBadge } from "../climatiq/ClimatiqFactorInput";
import ActivitySelection from "../ocel/selections/ActivitySelection";
import AttributeMultiSelection, { AttributeSourceDefinition } from "../ocel/selections/AttributeMultiSelection";
...
const EmissionRuleForm: React.FC<EmissionRuleFormProps> = ({ index, rule, setRule, getId }) => {
  const session = useOceanStore.use.session()
  const ocel = useOceanStore.use.ocel()

  const [availableEventAttributes, setAvailableEventAttributes] = useState<EventAttributeDefinition[] | undefined>()
  const [availableObjectAttributes, setAvailableObjectAttributes] = useState<QualifiedObjectAttributeDefinition[] | undefined>()
  const [isAvailableAttributesLoading, setIsAvailableAttributesLoading] = useState(false)

  const climatiqConfig = useOceanStore.use.climatiqConfig()

  const [climatiqSearchConfig, setClimatiqSearchConfig] = useState<ClimatiqEmissionFactorSearchConfig>({
    isActive: false
  })
  useEffect(() => {
  ...
  const matchingInputClimatiqUnitTypes = useMemo(() => {
    const dims: UnitTypeDim[] = rule.factor?.attributes?.map(attr => attr.attribute.unit?.dim ?? {}) ?? []
    const dimNames: string[] = _.uniq(dims.flatMap(dim => Object.keys(dim)))
    // compute sum of exponents
    const productDim: UnitTypeDim = Object.fromEntries(dimNames.map(dimName => [
      dimName,
      _(dims).map(dim => _.get(dim, dimName, 0)).sum()
  ...
  useEffect(() => {
    async function effect() {
      ...
      const res = await Api.getAvailableAttributesForEmissionRuleGetAvailableAttributesPost({
      ...
      setAvailableEventAttributes(_.sortBy(res.availableEventAttributes, attribute => [attribute.name]))
      setAvailableObjectAttributes(_.sortBy(res.availableObjectAttributes.map(t => {
        const [ot, q, attr] = t as unknown as [string, string | null, ObjectAttributeDefinition]
        return {
          objectType: ot,
          qualifier: q,
          attribute: attr
        }
      }), ({ objectType, qualifier, attribute }) => [attribute.name, objectType, !!qualifier, qualifier]))
  ...
  return (<>
    ...
    <Form.Group as={Row} className="mb-2">
      <Form.Label column sm={2} className="d-none d-sm-block">Type</Form.Label>
      <Col sm={10} className="d-flex flex-column justify-content-around">
        <Form.Check
          ...
          onChange={e => {
            const prev = rule.type == "EventEmissionRule" ? rule : (() => {
    ...
    <Form.Group as={Row} className="mb-2" controlId={getId("Activity")}>
      <Form.Label column sm={2} className="d-none d-sm-block">Activity</Form.Label>
      <Col sm={10} className="d-flex flex-column justify-content-around">
        <ActivitySelection
          id={getId("Activity")}
    ...
    {(rule.activity && rule.type == "E2OEmissionRule") && (<>
      ...
      <Form.Group as={Row} className="mb-2" controlId={getId("ObjectType")}>
        <Form.Label column sm={2} className="d-none d-sm-block">Object type</Form.Label>
        <Col sm={10} className="d-flex flex-column justify-content-around">
          <ObjectTypeSelection
            id={getId("ObjectType")}
      ...
      {/* ----- Qualifier ----------------------------------------------------------- */}
      {rule.objectType && (<>
        <Form.Group as={Row} className="mb-2" controlId={getId("Qualifier")}>
          <Form.Label column sm={2} className="d-none d-sm-block">Qualifier</Form.Label>
          <Col sm={10} className="d-flex flex-column justify-content-around">
            <QualifierSelection
              id={getId("Qualifier")}
              qualifiers={getQualifiers(ocel, { activity: rule.activity, objectType: rule.objectType })}
    ...
    {(rule.activity && (rule.type != "E2OEmissionRule" || rule.objectType)) && (<>
      ...
      <Row>
        <Col>
          <Form.Group as={Row} className="mb-2" controlId={getId("EventAttribute")}>
            <Form.Label column md={2}>Proportional to</Form.Label>
            <Col md={10}>
              {!isAvailableAttributesLoading && (
                <AttributeMultiSelection
                  ...
                  isDisabled={!availableEventAttributes?.length && !availableObjectAttributes?.length}
                  selected={rule.factor ? getSelectedAttributeSources(rule.factor) : []}
                  isClearable={true}
              ...
              <Form.Text id={`multiSelectHelpText-${index}`}>
                Either use the emission factor as a constant value, or select one or multiple attributes. When selecting multiple, their values and the emission factor get multiplied.
              </Form.Text>
            </Col>
          </Form.Group>
        </Col>
      ...
      <Row>
        <Col>
          <Form.Group as={Row} className="mb-2" controlId={getId("Quantity")}>
            <Form.Label column md={2}>
              Emission factor
              {(rule.factor?.source == "climatiq" && !!rule.factor.data) && (
                <a href={`https://www.climatiq.io/data/emission-factor/${rule.factor.data.id}`} target="_blank" className="ms-2" title={rule.factor.data.name}>
                  <FaArrowUpRightFromSquare />
                </a>
              )}
            </Form.Label>
            <Col md={10} className="d-flex flex-column justify-content-around gap-2">
              <div className="d-flex gap-2">
                <ButtonGroup>
                  <ToggleButton
                    ...
                    onChange={e => setRule({
                      ...rule,
                      factor: { source: "local", value: { value: 1, unit: KILOGRAM }, attributes: rule.factor?.attributes ?? [] }
                    })}
                  >
                  ...
                  <ToggleButton
                    ...
                    onChange={e => setRule({
                      ...rule,
                      factor: { source: "climatiq", data: undefined, attributes: rule.factor?.attributes ?? [] }
                    })}
                  >
                    <WithTooltip tooltip={climatiqSearchConfig.isActive ? "Climatiq emission factor search" : "Climatiq emission factor search (first enter API key)"}>
                      <ClimatiqLogoIcon
                        // disabled={!climatiqSearchConfig.isActive}
                        variant="color"
                ...
                <div className="d-flex align-items-center gap-1 w-100">
                  ...
                  {(!rule.factor || rule.factor.source == "local") && (
                    <QuantityInput
                      ...
                      value={rule.factor?.value ?? { unit: KILOGRAM, value: 1 }}
                      onChange={x => {
                        let factor: EditingEmissionFactor | undefined = undefined
                        if (rule.factor) {
                          factor = { attributes: [], ...rule.factor, source: "local", value: x }
                        } else if (x) {
                          factor = { source: "local", value: x, attributes: [] }
                        }
                        setRule({
                          ...rule,
                          factor: factor
                  ...
                  {(rule.factor?.source == "climatiq" && climatiqSearchConfig.unitTypeFilter !== undefined) && (<>
                    <FaFilter className="text-secondary me-2" />
                    {_.orderBy(climatiqSearchConfig.unitTypeFilter, ut => ut.length).map((ut, i) => (
                      <ClimatiqUnitTypeFilterBadge
                        key={i}
                        unitType={ut}
                        onRemove={() => setClimatiqSearchConfig(cnf => ({ ...cnf, unitTypeFilter: cnf.unitTypeFilter?.filter(ut1 => ut1 != ut) }))}
                      />
                    ))}
                    {climatiqSearchConfig.unitTypeFilter.length == 0 && (
                      <Badge bg="danger" text="light" className="d-flex align-items-center gap-1">
                        <FaBan />
                        Invalid unit type
                      </Badge>
                    )}
              ...
              {rule.factor?.source == "climatiq" && (
                <div>
                  <ClimatiqFactorInput
                    ...
                    selected={rule.factor.data as ClimatiqEmissionFactor | undefined}
                    onChange={climatiqFactor => {
                      let factor: EditingEmissionFactor | undefined = undefined
                      const newClimatiqFactor = climatiqFactor as unknown as ClimatiqEmissionFactorDetails | undefined
                      if (rule.factor) {
                        factor = { attributes: [], ...rule.factor, source: "climatiq", data: newClimatiqFactor }
                      } else if (climatiqFactor) {
                        factor = { source: "climatiq", data: newClimatiqFactor, attributes: [] }
                      }
                      setRule({
                        ...rule,
                        factor: factor
...
export function getSelectedAttributeSources(ef: EditingEmissionFactor): AttributeSourceDefinition[] {
