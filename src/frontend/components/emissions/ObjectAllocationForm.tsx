...
const ObjectAllocationForm: React.FC<{
  ...
  const rules: Rule[] = ["AllTargets", "ParticipatingTargets", "ClosestTargets"]
  const ruleOptions = Object.fromEntries(rules.map(r => [r, { label: r, value: r }]))
  ...
  useEffect(() => {
    const buildConfig = () => {
      ...
      if (rule == "AllTargets" || rule == "ParticipatingTargets")
        return base
  ...
  const hasBeenApplied = useMemo(() => {
    ...
    return _.isEqual(removeNull(objectEmissionResults?.objectAllocationConfig), removeNull(objectAllocationConfig))
  ...
  const triggerObjectAllocation = async () => {
    ...
    // })
    const data = await apiWrapper(() => Api.objectAllocationObjectAllocationPost({
      oceanSessionId: session,
      requestBody: {
  ...
  return (
    <Form>
      <Form.Group as={Row} className="mb-2" controlId="objAllocForm-targets">
        <Form.Label column sm={2}>Target objects</Form.Label>
      ...
      <Form.Group as={Row} className="mb-2" controlId="objAllocForm-rule">
        <Form.Label column sm={2}>Rule</Form.Label>
        <Col sm={10}>
          <Select<RuleOption>
            options={Object.values(ruleOptions)}
            value={ruleOption}
            onChange={it => { if (it) setRuleOption(it) }}
      ...
      <Form.Group as={Row} className="mb-2">
        <Col sm={2}></Col>
        <Col sm={10}>
          <Button variant="primary" disabled={!objectAllocationConfig || !session || hasBeenApplied} onClick={triggerObjectAllocation}>Apply</Button>
