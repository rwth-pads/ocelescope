...
const defaultNoUnitNumberFormat = new Intl.NumberFormat(
  defaultLocales,
...
export function quantityString(
  ...
  if (unit && isMoney(unit)) {
    ...
    try {
      ...
      console.error(
        `quantityString() failed with style: "currency"`,
        x,
        options,
      );
    }
  ...
  // Render number + unit
  const noUnitNumberFormat = !_.isEmpty(numberFormatOptions)
...
const Qty: React.FC<QtyProps> = ({
  ...
  if (value === undefined || str == "-") {
    return (
      <span
        className={combineClassNames(className, "text-secondary")}
        {...props}
      >
        -
      </span>
...
export const QtyRange: React.FC<QtyRangeProps> = ({
  ...
  if (value1.value == value2.value && summarizeEqual) {
    return (
      <Qty value={value1} options={options} className={className} {...props} />
    );
  }
