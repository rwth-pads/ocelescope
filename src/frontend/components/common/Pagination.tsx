...
const breakpoints = _.intersection(
  Object.keys(edgeDelta),
  Object.keys(centerDelta),
);
...
> = ({ min = 1, max, current, setCurrent, className, ...props }) => {
  ...
  const visiblePages = Object.fromEntries(
    breakpoints.map((bp) => {
      if (!(bp in edgeDelta && bp in centerDelta)) {
        return [bp, []];
      }
      const e = edgeDelta[bp],
        c = centerDelta[bp];
  ...
  const visiblePagesWithEllipses = Object.fromEntries(
    Object.entries(visiblePages).map(([bp, ps]) => [
      bp,
      [
