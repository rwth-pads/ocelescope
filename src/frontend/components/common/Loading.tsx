...
const rippleAnimation = (props: SpinnerComponentProps) => keyframes`
  0% {
    top: ${props.$size * (.5 - .5 * .05)}px;
    left: ${props.$size * (.5 - .5 * .05)}px;
    width: 0;
    height: 0;
    opacity: 0;
  }
...
export const Ripple = generateComponent(2, styled.div<SpinnerComponentProps>`
  ...
  div {
    position: absolute;
    border: ${props => .05 * props.$size}px solid ${props => props.$color};
    opacity: 1;
    border-radius: 50%;
...
export const Spinner = generateComponent(12, styled.div<SpinnerComponentProps>`
  color: official;
  display: inline-block;
  position: relative;
  width: ${props => props.$size}px;
  height: ${props => props.$size}px;
  div {
    transform-origin: ${props => .5 * props.$size}px ${props => .5 * props.$size}px;
