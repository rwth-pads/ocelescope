...
const Histogram: React.FC<HistogramProps> = ({
  ...
  const yScale = useMemo(() => {
    ...
    return scale
      ...
      .domain([preMap(0), preMap(maxSize)])
      .nice();
  ...
  useEffect(() => {
    ...
    const { numXTicks, numYTicks } = getNumberOfTicks(
      boundsWidth,
      boundsHeight,
      xScale.domain()[0],
      xScale.domain()[1],
    ...
    axes
      ...
      .attr("font-size", `${ticksFontSize}px`)
  ...
  const allRects = buckets.map((bucket, i) => {
    if (bucket.x0 === undefined || bucket.x1 === undefined) return false;

    const binCenter = xScale((bucket.x1 + bucket.x0) / 2);
    const binWidth =
      bucket.x1 != bucket.x0
        ? xScale(bucket.x1) - xScale(bucket.x0) - BAR_GAP
        : 50;
      ...
      <OverlayTrigger
        ...
        overlay={
          <Tooltip id={`bin-tooltip-${i}`}>
            ...
            <span>
              ({bucket.length}&nbsp;{getItemCountName(bucket.length)})
            </span>
          </Tooltip>
        }
      >
        <rect
          ...
          height={
            yScale(logScale.y ? 1 : 0) -
            yScale(bucket.length + (logScale.y ? 1 : 0))
          }
          opacity={highlightedBin == i || highlightedBin === null ? 1 : 0.5}
        />
      </OverlayTrigger>
    );
  });
      ...
      <svg width={width} height={height}>
        ...
        {yLabel && (
          <text
            ...
            y={0}
            textAnchor="middle"
            transform={`translate(12,${MARGIN.top + boundsHeight / 2}),rotate(-90)`}
          >
            {yLabel}
          </text>
        )}
