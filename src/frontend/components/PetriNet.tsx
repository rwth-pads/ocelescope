...
import {
  ...
  renderOcpn as ocpnToDot,
} from "@/src/ocpn.ts-graphviz";
...
const ActivityEmissionLabel: React.FC<
...
> = ({
  ...
  const w = 40,
    h = 30;
  const { x, y } = alignAtBBox(bbox, "S", w, h, 0.25, true);
  const textOffset = 4;
...
const AdditionalComponents: React.FC<{
  ...
  return (
    <>
      ...
      {emissions &&
        activityEmissionColorScale &&
        ocel.activities.map((activity, i) => {
          const thisActivityEmissions = emissions.activityEmissions[activity];
          if (thisActivityEmissions.nonzero == 0) return false;
          const activityTransition = graph
            .children("g")
            .filter(`[id='activity-transition-${activity}']`);
          if (!activityTransition.length) return false;
          return (
            <ActivityEmissionLabel
              key={i}
              activity={activity}
...
const PetriNet: React.FC<PetriNetProps> = ({
  ocpn,
  emissions,
  options,
  ...graphvizProps
  ...
  useEffect(() => {
    if (!ocel) return;
    setDot(
      ocpnToDot(ocpn, ocel, objectTypeColors, emissions, {
  ...
  const activityEmissionColorScale = useMemo(
    () =>
      emissions
        ? getEmissionColorScale(
    ...
    <div id={id} ref={containerRef}>
      {!!dot && (
        <>
          <GraphvizContainer
            dot={dot}
            options={{ ...options, width: containerDimensions.width }}
            {...graphvizProps}
          />

          {!!(svg && svg.length && graph && graph.length) &&
            createPortal(
...
export function getBoundingBox(el: JQuery<SVGElement>): BBox | undefined {
  ...
  if (tagName == "polygon") {
    const points = el
      .attr("points")
      ?.split(" ")
      .map((t) => t.split(",").map((s) => Number.parseFloat(s)));
    if (
      !points ||
      !points.length ||
      !points.every((p) => p.length == 2 && p.every((x) => !isNaN(x)))
    ) {
      return undefined;
    }
...
export function alignAtBBox(
  bbox: BBox,
  anchor: Anchor,
  width: number,
  height: number,
  overlap: number = 0,
  relativeOverlap: boolean = false,
): { x: number; y: number } {
  if (relativeOverlap) {
  ...
  if (anchor == "N") {
    return {
      x: (bbox.x[0] + bbox.x[1] - width) / 2,
      y: bbox.y[0] + overlap - height,
    };
  }
  if (anchor == "E") {
  ...
  if (anchor == "W") {
    return {
      x: bbox.x[0] + overlap - width,
      y: (bbox.y[0] + bbox.y[1] - height) / 2,
    };
  }
  if (anchor == "C") {
    return {
