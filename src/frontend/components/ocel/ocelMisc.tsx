...
export const AttrValue: React.FC<{
  ...
  const attr = ocel?.attributes.find((attr) => {
    ...
    if (event && isEventAttribute(attr) && attr.activity == event.activity)
      return true;
    if (object && isObjectAttribute(attr) && attr.objectType == object.type)
      return true;
    return false;
  });
  ...
  if (!ocel || !values || !attr || !(name in values))
