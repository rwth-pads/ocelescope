...
}> = ({ handleSampleEvents, handleSampleObjects, apiWrapper }) => {
  const ocel = useOceanStore.use.ocel();
  const emissions = useOceanStore.use.emissions();
  const { setIsEmissionAttributeSelectionOpen } =
    useOceanStore.useState.isEmissionAttributeSelectionOpen();
        ...
        <>
          <div className="d-flex flex-column gap-1">
            <div className="d-flex align-items-center gap-2">
              ...
              <span>
                File name: <code>{ocel.meta.fileName}</code>
              </span>
            </div>
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              <span className="me-2">
                {ocel.numEvents} Events of {ocel.activities.length} activities
              </span>
              {!!handleSampleEvents && (
                <Button
                  size="sm"
                  onClick={() => handleSampleEvents()}
                  variant="light"
                >
                  Show sample
                </Button>
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              <span className="me-2">
                {ocel.numObjects} Objects of {ocel.objectTypes.length} types
              </span>
              {!!handleSampleObjects && (
                <Button
                  size="sm"
                  onClick={() => handleSampleObjects()}
                  variant="light"
                >
                  Show sample
                </Button>
            ...
            <div className="d-flex align-items-center gap-2">
              <FaCloud
                className={!!emissions ? "text-success" : "text-secondary"}
              />
              <span className="me-2">
                {!!emissions && (
                  <>
                    Total emissions:{" "}
                    <Qty
                      value={emissions.overallEmissions}
                      unit={emissions.unit}
                    />
                  </>
                )}
                {!emissions && <>No emission data</>}
              </span>
              {!emissions && (
                <Button
                  size="sm"
                  onClick={() => setIsEmissionAttributeSelectionOpen(true)}
                  variant="light"
                >
                  Select attributes
                </Button>
