...
import { attributeDefinitionEquals, isCategoricalAttribute, isEventAttribute, isNumericAttribute, isNumericEventAttribute, isObjectAttribute, OCELAttribute, OCELCategoricalAttribute, OCELEventAttribute, OCELNumericAttribute, OCELObjectAttribute } from '@/src/ocel.types';
...
import { ActivityWithIcon, AttributeName, ObjectTypeWithIcon, pluralize } from '@/components/misc';
import { AttributeDefinition, findOcelAttributeUnitDefinition, useAttributeUnitState } from '@/src/app-state.types';
 ...
 */
const isValidEmissionAttribute = (attr: OCELAttribute, def: AttributeDefinition | undefined) => {
 ...
 */
const isValidEventEmissionAttributeInput = (attr: OCELAttribute, def: AttributeDefinition | undefined) => {
...
const unitSelectionProps: Omit<UnitSelectionProps, "units"> = {
  ...
  styles: {
    ...
    valueContainer: { minWidth: "40px", fontSize: ".9em", paddingLeft: "4px", paddingRight: "4px" },
...
export const AttributeManager = styled(({
  ...
  const attrNameGroups = _.groupBy(_.sortBy(attributes, attr => attr.name), attr => attr.name)
  const [expandedAttrNameGroups, setExpandedAttrNameGroups] = useState<string[]>([])

  const emissionAttributes = useOceanStore.use.emissionAttributes()
  const currentEmissionAttributes = editingEmissionAttributes ?? emissionAttributes

  // const _unitStates = attributes.map(attr => useAttributeUnitState(attr))
  const attributeStates = Object.fromEntries(Object.entries(attrNameGroups).map(([name, attrNameGroup]) => [
    name,
    attrNameGroup.map(attr => {
      const attrDefinition = findOcelAttributeUnitDefinition(attr, attributeUnits)
      const attrIsValid = isValidEventEmissionAttributeInput(attr, attrDefinition)
      const isEmissionAttribute = currentEmissionAttributes.some(emAttr => {
        return attributeDefinitionEquals(attr, emAttr)
      })

      const setIsEmissionAttribute = (attrIsValid && attrDefinition && setEditingEmissionAttributes) ? (b: boolean) => {
        setEditingEmissionAttributes(emAttrs => {
          const others = emAttrs.filter(emAttr => !attributeDefinitionEquals(attr, emAttr))
          if (b) return [
            ...others,
            attrDefinition
          ]
      ...
      return {
        ...
        checkboxState: {
          ...
          disabledTooltip: !attrIsValid ? "First assign a weight unit" : undefined,
  ...
  const rowContents: { cells: JSX.Element, props: HTMLProps<HTMLTableRowElement> }[] = []

  Object.entries(attrNameGroups).forEach(([name, attrNameGroup], i) => {

    const isExpanded = expandedAttrNameGroups.includes(name) || attrNameGroup.length == 1
    const toggleExpanded = () => setExpandedAttrNameGroups(names => names.includes(name) ? names.filter(n => n != name) : [...names, name])
  ...
  return (<div className="mb-3">
    <Table striped size="sm" className={combineClassNames("align-middle", className)} {...props}>
      <thead>
        <tr>
          ...
          {selectEmissionAttributes && <th className="emissions-cell"><FaCloud /></th>}
        </tr>
      </thead>
      <tbody>
        {rowContents.map(({ cells, props }, k) => <tr key={k} {...props}>{cells}</tr>)}
      </tbody>
    </Table>
  ...
  selectEmissionAttributes
    ...
    return (<>
      ...
      <td className="types-cell">
        {attr.target == "event" && <ActivityWithIcon activity={attr.activity} />}
        {attr.target == "object" && <ObjectTypeWithIcon objectType={attr.objectType} />}
      </td>
      <td className="num-values-cell">
        {attr.numValues}
      </td>
      <td className="range-cell d-none d-md-table-cell">
        {isNumericAttribute(attr) && <NumericAttributeRange attr={attr} unit={unit} />}
        {isCategoricalAttribute(attr) && <CategoricalAttributeRange attr={attr} maxBadgeWidth="100px" />}
      </td>
      <td className="unit-cell">
        {(attr.numeric && setUnit) && (
          <WithTooltip tooltip={isEmissionAttribute ? "Emissions units cannot be changed" : undefined}>
            <UnitEditor
              value={unit}
...
const AttributeNameGroupRow: React.FC<{
  ...
  attributeStates
    ...
    const numActivities = attrNameGroup.filter(attr => isEventAttribute(attr)).length
    const numObjectTypes = attrNameGroup.filter(attr => isObjectAttribute(attr)).length

    const uniqueUnits = _.uniqWith(attributeStates, ({ unit: u1 }, { unit: u2 }) => {
    ...
    const areSomeEmissionAttributes = _.some(attributeStates, "isEmissionAttribute")
    ...
    return (
      <>
        ...
        <td className="name-cell ms-0">
          {attrNameGroup.length > 1 ? <span onClick={() => toggleExpanded()} style={{ cursor: "pointer" }}>{name}</span> : name}</td>
        <td className="types-cell">
          <div className="d-flex flex-column">
            {(target == "event" || (target == "both" && numActivities != 0)) && <span>{pluralize(numActivities, "activity", "activities")}</span>}
            {(target == "object" || (target == "both" && numObjectTypes != 0)) && <span>{pluralize(numObjectTypes, "object type", "object types")}</span>}
          </div>
        </td>
        <td className="num-values-cell">{_.sumBy(attrNameGroup, attr => attr.numValues)}</td>
        <td className="range-cell d-none d-md-table-cell">
          {showMergedRange && (<>
            {allNumeric && <NumericAttributeRange attr={{
              target: "event",
              activity: "<dummy>",
              numeric: true,
              name: name,
              numValues: _.sumBy(attrNameGroup, attr => attr.numValues),
              min: Math.min(...(attrNameGroup as OCELNumericAttribute[]).map(attr => attr.min)),
              max: Math.max(...(attrNameGroup as OCELNumericAttribute[]).map(attr => attr.max)),
              mean: NaN,
              median: NaN,
              type: uniqueTypes[0]
            }} unit={uniqueUnit} />}
        ...
        <td className="unit-cell">
          {canEditAllUnits && (
            <WithTooltip tooltip={areSomeEmissionAttributes ? "Emissions units cannot be changed" : undefined}>
              <UnitEditor
          ...
          {!canEditAllUnits && (<>
            {allCategorical && <small className="text-secondary">not numeric</small>}
...
export const NumericAttributeRange: React.FC<{
  ...
  return (<>
    <QtyRange value1={attr.min} value2={attr.max} unit={unit ?? undefined} options={{ showZero: true }} />
    {/* {(attr.min != attr.max) && (<>
...
export const CategoricalAttributeRange: React.FC<{
  ...
  return (
    <div className="d-flex gap-1 flex-wrap">
      ...
      {_.sortBy(Object.entries(attr.frequentValues), ([x, count]) => -count).map(([x, count]) => x).slice(0, numValuesShown).map((x, i) => {
        ...
        return (
          <Badge
            ...
            text="dark"
            key={i}
            style={maxBadgeWidth ? { maxWidth: maxBadgeWidth, overflow: "hidden", textOverflow: "ellipsis" } : undefined}
          >{x}</Badge>
        )
      })}
      {attr.numUnique > numValuesShown && (
        <Badge bg="secondary" text="light" className="">+{attr.numUnique - numValuesShown}</Badge>
      )}
    </div>
  )
}