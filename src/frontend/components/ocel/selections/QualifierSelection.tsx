...
const QualifierSelection: React.FC<{
...
}> = ({ id, qualifiers, selected = null, onChange, selectWhenUnique = false, disableWhenUnique = true, addUndefinedOption = true, placeholder = "Qualifier", disabled, styles }) => {
  ...
  return (
    <Select<QualifierItem>
      ...
      //   {item.label}
      // )}
      isDisabled={disabled || (disableWhenUnique ? qualifiers.length == 1 : false)}
      isClearable={false}
