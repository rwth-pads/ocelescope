...
import { AttributeDefinition, findOcelAttribute, useAttributeUnit } from "@/src/app-state.types";
import { isDynamicObjectAttribute, OCEL, OCELAttribute, QualifiedObjectAttributeDefinition } from "@/src/ocel.types";
import { combineClassNames } from "@/src/util";
import { useOceanStore } from "@/src/zustand";
import _ from "lodash";
import { FaEquals } from "react-icons/fa6";
import Select, { FormatOptionLabelMeta, Props as SelectProps } from "react-select";
import { AttributeName } from "../../misc";

export type AttributeSourceDefinition = EventAttributeDefinition | QualifiedObjectAttributeDefinition
...
const availabilityNumberFormat = new Intl.NumberFormat(undefined, { style: "percent", maximumSignificantDigits: 1 })
const otq = ({ objectType, qualifier, attribute }: QualifiedObjectAttributeDefinition) => !!qualifier ? `${objectType}/${qualifier}` : objectType
...
const makeItem = (
  ...
  const availability = isDynamicObjectAttribute(attr) && showAvailabilityForActivity ? _.get(attr.availability, showAvailabilityForActivity) : undefined
  return {
    label: `${attr.name}`
      + (unit ? ` [${unit.symbol}]` : "")
      + ("objectType" in s ? ` (${otq(s)})` : "")
      + (availability !== undefined ? ` (available ${availabilityNumberFormat.format(availability)})` : ""),
    attribute: attr,
    attributeUnit: def,
    attributeSource: s,
    value: attr.name
...
type AttributeMultiSelectionProps = Omit<SelectProps, "id" | "inputId" | "classNamePrefix" | "selected" | "options" | "onChange" | "defaultValue" | "styles"> & {
  id?: string
...
const AttributeMultiSelection = ({
  ...
  const noneItem = makeItem(undefined, ocel, attributeUnits, constLabel, showAvailabilityForActivity)
  const items = [
    ...(includeNoneOption ? [noneItem] : []),
    ...eventAttributes.map(ea => makeItem(ea, ocel, attributeUnits, constLabel, showAvailabilityForActivity)),
    ...objectAttributes.map(oa => makeItem(oa, ocel, attributeUnits, constLabel, showAvailabilityForActivity))
  ]
  ...
  return (
    <Select<AttributeItem, true>
      ...
      onChange={(items, actionMeta) => {
        if (onChange) {
          if (actionMeta.action == "select-option" && !actionMeta.option?.attributeSource) {
            // select "const" - clear all
            console.log(`select "const" - clear all`)
            onChange([])
          } else {
            console.log(`selected ${items.length} items`)
            const attrSources = items?.map(it => it.attributeSource).filter(s => s) as AttributeSourceDefinition[] ?? []
            onChange(attrSources)
          }
        }
      }}
      formatOptionLabel={(item: AttributeItem, { context, inputValue, selectValue }: FormatOptionLabelMeta<AttributeItem>) => !!item.attribute ? (
