...
const ObjectTypeSelection: React.FC<{
...
}> = ({ id, objectTypes, selected, onChange, addUndefinedOption = false, isMulti = false, styles }) => {
  ...
  return (
    <Select<ObjectTypeItem>
      ...
      options={items}
      formatOptionLabel={(option: ObjectTypeItem, { context, inputValue, selectValue }: FormatOptionLabelMeta<ObjectTypeItem>) => (
        option.objectType ? <ObjectTypeWithIcon objectType={option.objectType} /> : <FaCircle className="text-secondary" />
      )}
