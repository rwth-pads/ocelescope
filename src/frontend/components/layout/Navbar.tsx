...
const Navbar: React.FC<{
  ...
  const { setIsClimatiqSettingsOpen } =
    useOceanStore.useState.isClimatiqSettingsOpen();
  const [emissionDownloadMode, setEmissionDownloadMode] =
    useState<EmissionDownloadMode>(false);
  ...
  const triggerDownload = useCallback(
    ({ emissionDownloadMode = false }: DownloadOptions) => {
      async function callback() {
        if (!ocel || !session || !apiState) return false;
        setLoadingText("Preparing download");
        const params = emissionDownloadMode
          ? `?emissions=${emissionDownloadMode}`
          : "";
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/download${params}`,
          {
            headers: {
        ...
        {session && ocel && apiState && (
          <>
            ...
            <NavDropdown title={<FaGear />} id="navbarScrollingDropdown">
              <NavDropdown.Item
                onClick={() => setIsClimatiqSettingsOpen(true)}
            ...
            <Button
              ...
              onClick={async () => {
                if (
                  await confirm("Are you sure you want to leave the session?", {
            ...
            <Modal.Body>
              {hasEventEmissions && hasObjectEmissions && (
                <Alert variant="info" className="small">
                  Emissions can be exported on event or object level,{" "}
                  <strong>not</strong> both at once. This way, overall process
              ...
              <Form.Check
                ...
                onChange={(e) =>
                  setEmissionDownloadMode(e.target.checked ? "events" : false)
                }
              />
              <Form.Check
                type="switch"
                ...
                onChange={(e) =>
                  setEmissionDownloadMode(e.target.checked ? "objects" : false)
                }
              />
            </Modal.Body>
            <Modal.Footer>
