...
const ClimatiqSettingsModal: React.FC<ClimatiqSettingsModalProps> = ({
  ...
  const { climatiqConfig, setClimatiqConfig } = useOceanStore.useState.climatiqConfig()
  const { isClimatiqSettingsOpen, setIsClimatiqSettingsOpen } = useOceanStore.useState.isClimatiqSettingsOpen()
  const [dataVersionInput, setDataVersionInput] = useState(climatiqConfig.dataVersion?.toString() ?? "")
  ...
  return (
    <Modal show={isClimatiqSettingsOpen} size="lg" fullscreen="xs-down" onHide={() => setIsClimatiqSettingsOpen(false)}>
      <Modal.Header closeButton>
        <Modal.Title className="d-flex align-items-center gap-2">
          <a href="https://climatiq.io" target="_blank" title="Climatiq website">
            <ClimatiqLogoIcon />
          </a>
          Climatiq Config
        </Modal.Title>
      ...
      <Modal.Body>
        <Form.Group as={Row} className="mb-2" controlId="climatiq-config-api-key">
          <Form.Label column sm={2} className="d-none d-sm-block">API key</Form.Label>
          <Col sm={10}>
            <Form.Control
              type="password"
        ...
        <Form.Group as={Row} className="mb-2 align-items-center" controlId="climatiq-config-data-version">
          <Form.Label column sm={2} className="d-none d-sm-block">Data version</Form.Label>
          <Col sm={10} className="d-flex">
            <Form.Control
              disabled={process.env.NEXT_PUBLIC_CLIMATIQ_DATA_VERSION !== undefined}
      ...
      <Modal.Footer>
        <Button variant="secondary" onClick={() => setIsClimatiqSettingsOpen(false)}>Close</Button>
      </Modal.Footer>
    </Modal>
  )
...
export const ClimatiqLogoIcon: React.FC<Omit<ImageProps, "src" | "alt"> & {
  disabled?: boolean
  variant?: "color" | "white"
}> = ({
  disabled = false,
  ...
  const size = 30
  const src = variant == "white" ? "/app/Climatiq_Icon_white.png" : "/app/Climatiq_Icon.png"
