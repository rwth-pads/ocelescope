...
import { ClimatiqEmissionFactor, climatiqSearchRequest, ClimatiqSearchRequest } from "@/src/climatiq.types"
...
const ClimatiqFactorInput: React.FC<ClimatiqFactorInputProps> = ({
  ...
  const loadOptions = async (inputValue: string): Promise<EmissionFactorOption[]> => {
    ...
    if (response === false) {
      setErrorMessage("Climatiq search request failed. Make sure to enter a valid API key in the settings.")
      return []
    }
    const results = _.orderBy(response.results, ef => [
      ef.year
    ], "desc")
  ...
  return (
    <AsyncSelect<EmissionFactorOption>
      ...
      onChange={onChange ? option => onChange(option?.emissionFactor) : undefined}
      defaultOptions
      loadOptions={debouncedLoadOptions}
      cacheOptions={true}
      noOptionsMessage={({ inputValue }) => inputValue.length >= minSearchLength ? `No results for "${inputValue}"` : "Search for an emission factor"}
      formatOptionLabel={({ emissionFactor: ef }: EmissionFactorOption, { context, inputValue, selectValue }: FormatOptionLabelMeta<EmissionFactorOption>) => {
        return <div className="d-flex align-items-center gap-2">
          <RegionIcon region={ef.region} size={20} />
          <span>{ef.name}</span>
          <span
            className="ms-auto text-secondary font-monospace"
            style={{ fontSize: ".8em", overflow: "hidden", textOverflow: "ellipsis" }}
          >({ef.id})</span>
          <div className="">
            <WithTooltip tooltip={`Unit type: ${ef.unitType}`}>
              <UnitTypeIcon climatiqName={ef.unitType} className="text-secondary small" />
            </WithTooltip>
          </div>
        </div>
      }}
...
export const ClimatiqUnitTypeFilterBadge = styled(({
  ...
  onRemove,
  ...props
}: ClimatiqUnitTypeFilterBadgeProps) => {
  return (
